...
void *realloc(void *oldptr, size_t size) {
    ...
    if(!(*(unsigned*)next&USED))
    {
        if(nextlen>=newlen-oldlen)
        {
            remove_empty_block(next);
            if(nextlen>=newlen-oldlen+16)
            {
                *(unsigned*)p=newlen|msk;
                next=next+(newlen-oldlen);
                nextlen=nextlen-(newlen-oldlen);
    ...
    {
        next=extend_heap(newlen-oldlen);
        remove_empty_block(next);
        *(unsigned*)tail|=PREV;
        *(unsigned*)p=(tail-p)|msk;
        return oldptr;
    }
