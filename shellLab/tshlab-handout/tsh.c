...
volatile sig_atomic_t fg_fn;// whether a foreground job is running
...
// check whether string s is a number, save the number in num, save sign in sgnp
// return 0 if it's job id, 1 if it's pid, -1 if it's invalid
int checkid(char *s,int *num,int *sgnp)
{
    if(!s)
        return -1;
    *num=0;
...
eval(char *cmdline) 
{
    ...
    {
        int fout=tok.outfile?open(tok.outfile,O_WRONLY|O_CREAT|O_TRUNC)
            :STDOUT_FILENO;
    ...
    else if(tok.builtins==BUILTIN_KILL)
    {
        ...
        int id,typ,sgn;
        // invalid argument
        if((typ=checkid(tok.argv[1],&id,&sgn))==-1)
        {
        ...
        // get pointer to job
        struct job_t *jb=typ?getjobpid(job_list,id):
            getjobjid(job_list,id);
        if(jb)
        {
            // send SIGTERM
            kill(sgn*jb->pid,SIGTERM);
        }
        // job not found
        else
        {
    ...
    else if(tok.builtins==BUILTIN_BG)
    {
        ...
        // get pointer to job
        struct job_t *jb=typ?getjobpid(job_list,id):getjobjid(job_list,id);
        if(jb)
        {
            printf("[%d] (%d) %s\n",jb->jid,jb->pid,jb->cmdline);
            fflush(stdout);
            jb->state=BG;
            // send SIGCONT
            kill(-jb->pid,SIGCONT);
    ...
    else if(tok.builtins==BUILTIN_FG)
    {
        ...
        if(jb)
        {
            ...
            jb->state=FG;
            sigfillset(&chdmsk);
            sigdelset(&chdmsk,SIGCHLD);
            sigdelset(&chdmsk,SIGINT);
            sigdelset(&chdmsk,SIGTSTP);
            // send SIGCONT
            kill(-jb->pid,SIGCONT);
            // wait for foreground process to exit
            while(fg_fn)
                sigsuspend(&chdmsk);
        }
        else
    ...
    else if(tok.builtins==BUILTIN_NOHUP)
    {
        ...
        if((pid=fork())==0)
        {
            ...
            if(tok.infile)
            {
                ...
                if(fd<0)
                {
                    fprintf(stderr,"Error: %s No such file or directory",tok.infile);
                    fflush(stderr);
                    exit(0);
                }
            ...
            if(tok.outfile)
            {
                int fd=open(tok.outfile,O_WRONLY|O_TRUNC|O_CREAT);
                dup2(fd,STDOUT_FILENO);
            }
            // ignore SIGHUP
            sigaddset(&prev,SIGHUP);
        ...
        if(!bg)
        {
            fg_fn=1;
            sigset_t chdmsk;
            // can only be interrupted by sigchld, sigint and sigtstp
            sigfillset(&chdmsk);
            sigdelset(&chdmsk,SIGCHLD);
            sigdelset(&chdmsk,SIGINT);
            sigdelset(&chdmsk,SIGTSTP);
        ...
        {
            struct job_t *jb=getjobpid(job_list,pid);
            printf("[%d] (%d) %s\n",jb->jid,pid,cmdline);
            fflush(stdout);
        }
    }
